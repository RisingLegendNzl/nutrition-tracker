...
const PLAN_STORAGE_KEY = 'nutrify_mealPlan';
...
(function ensureMealPlan(){
  ...
  if (!window.mealPlan) window.mealPlan = { Monday: [] };
})();
...
export function renderDiet(){
  ...
  if (!mealsRaw.length){
    mealsEl.innerHTML = `<div class="empty-state">
      ...
      <div class="empty-sub">Generate a plan from your Profile, then return here.</div>
    </div>`;
    return;
  }
  ...
  // Persist latest plan view (optional)
  try { localStorage.setItem(PLAN_STORAGE_KEY, JSON.stringify(window.mealPlan)); } catch {}
}
